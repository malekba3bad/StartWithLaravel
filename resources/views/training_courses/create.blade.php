...
<div class="col-md-12">
<form method="POST" action="{{ route('training_courses.store') }}" style="width: 80%; margin: 0 auto; background-color:white">
    @csrf
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" name="price" class="form-control" oninput="this.value=this.value.replace(/[^0-9.]/g,'');" id="price" placeholder="سعر الدورة" value="{{ old('price') }}">

                  </div>
                  <div class="form-group">
                    <label for="start_date">تاريخ بدء الدورة</label>
                    <input type="date" name="start_date" class="form-control" id="start_date" placeholder="تاريخ بدء الدورة" value="{{ old('start_date') }}">
                    @error('start_date')
                    <div style="color: red" class="alert alert-danger">{{ $message }}</div>
                    @enderror
                  </div>
                   <div class="form-group">
                    ...
                    <input type="date" name="end_date" class="form-control" id="end_date" placeholder="تاريخ نهاية الدورة" value="{{ old('end_date') }}">
                    @error('end_date')
                    <div style="color: red" class="alert alert-danger">{{ $message }}</div>
                    @enderror
                  </div>
